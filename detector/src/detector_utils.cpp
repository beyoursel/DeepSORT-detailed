                  ...
                  ScaleInfo& scale_info)
{
    ...
    cv::Mat dst = cv::Mat::zeros(cv::Size(target_width, target_height), src.type());
                  ...
                  const std::vector<std::string>& classes)
{
    for (const auto& dr : results) {
        ...
        cv::rectangle(frame,
                      cv::Point(dr.box.tl().x, dr.box.tl().y - 20),
        ...
        cv::putText(frame, label, cv::Point(dr.box.x, dr.box.y + 6),
