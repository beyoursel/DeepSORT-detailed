...
void YOLOv26Detector::load_classes()
{
    classes_.clear();
...
void YOLOv26Detector::detect(cv::Mat& frame, std::vector<detect_result>& results)
{
    results.clear();
    ...
    if (output_shape.size() != 3 || output_shape[2] != 6) {
        throw std::runtime_error("Unexpected YOLOv26 output shape, expected (1, 300, 6). "
                                 "If you exported with end2end=False, use YOLOv8-style post-processing.");
    }
...
void YOLOv26Detector::draw_frame(cv::Mat& frame, std::vector<detect_result>& results)
{
