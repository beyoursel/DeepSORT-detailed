...
static std::string safeGetString(const YAML::Node& node, const std::string& key) {
    if (!node[key]) {
...
template <typename T>
static T getWithDefault(const YAML::Node& node, const std::string& key, const T& default_val) {
    if (!node[key]) {
        return default_val;
    }
    return node[key].as<T>();
...
bool AppConfig::load(const std::string& yaml_path) {
    try {
        ...
        if (config["input"]) {
            ...
            input.type = getWithDefault(config["input"], "type", std::string("video"));
        }

        // output
        if (config["output"]) {
            output.video = safeGetString(config["output"], "video");
            output.image = safeGetString(config["output"], "image");
            output.fourcc = getWithDefault(config["output"], "fourcc", std::string("MJPG"));
        ...
        if (config["detector"]) {
            ...
            detector.confidence_threshold = getWithDefault(config["detector"], "confidence_threshold", 0.25f);
            detector.nms_threshold = getWithDefault(config["detector"], "nms_threshold", 0.4f);
        }

        // tracker
        if (config["tracker"]) {
            tracker.type = getWithDefault(config["tracker"], "type", std::string("bytetrack"));
        }
        ...
        if (config["deepsort"]) {
            ...
            deepsort.max_iou_distance = getWithDefault(config["deepsort"], "max_iou_distance", 0.7f);
        ...
        if (config["bytetrack"]) {
            ...
            bytetrack.track_buffer = getWithDefault(config["bytetrack"], "track_buffer", 30);
            bytetrack.track_thresh = getWithDefault(config["bytetrack"], "track_thresh", 0.5f);
            bytetrack.high_thresh = getWithDefault(config["bytetrack"], "high_thresh", 0.6f);
            bytetrack.match_thresh = getWithDefault(config["bytetrack"], "match_thresh", 0.8f);
        }
