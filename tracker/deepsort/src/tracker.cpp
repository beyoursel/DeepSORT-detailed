...
void Tracker::InitiateTrack(const DETECTION_ROW& detection) {
  KAL_DATA data = kf->Initiate(detection.ToXyah());
  KAL_MEAN mean = data.first;
  KAL_COVA covariance = data.second;

  this->tracks.push_back(Track(mean, covariance, this->next_idx, this->n_init,
                               this->max_age, detection.feature));
  next_idx += 1;
}

DYNAMICM Tracker::GatedMatric(std::vector<Track>& tracks,
...
DYNAMICM
Tracker::IouCost(std::vector<Track>& tracks, const DETECTIONS& dets,
                  const std::vector<int>& track_indices,
                  const std::vector<int>& detection_indices) {
  ...
  for (int i = 0; i < rows; i++) {
    ...
    DETECTBOX bbox = tracks[track_idx].ToTlwh();
