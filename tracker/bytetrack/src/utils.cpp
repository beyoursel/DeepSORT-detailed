...
std::vector<std::vector<float>> ByteTracker::Ious(
  ...
  for (int k = 0; k < btlbrs.size(); k++) {
    ...
    for (int n = 0; n < atlbrs.size(); n++) {
      float iw = cv::min(atlbrs[n][2], btlbrs[k][2]) -
                 cv::max(atlbrs[n][0], btlbrs[k][0]) + 1;
      if (iw > 0) {
        float ih = cv::min(atlbrs[n][3], btlbrs[k][3]) -
                   cv::max(atlbrs[n][1], btlbrs[k][1]) + 1;
        if (ih > 0) {
...
double ByteTracker::Lapjv(const std::vector<std::vector<float>>& cost,
  ...
  int ret = LapjvInternal(n, cost_ptr, x_c, y_c);
  if (ret != 0) {
