...
void ByteKalmanFilter::Predict(KAL_MEAN& mean, KAL_COVA& covariance) {
  // revise the data;
  DETECTBOX std_pos;
  std_pos << std_weight_position_ * mean(3), std_weight_position_ * mean(3),
      1e-2, std_weight_position_ * mean(3);
  DETECTBOX std_vel;
  std_vel << std_weight_velocity_ * mean(3), std_weight_velocity_ * mean(3),
      1e-5, std_weight_velocity_ * mean(3);
  KAL_MEAN tmp;
  ...
  KAL_COVA covariance1 =
      this->motion_mat_ * covariance * (motion_mat_.transpose());
...
KAL_HDATA ByteKalmanFilter::Project(const KAL_MEAN& mean,
...
KAL_DATA
ByteKalmanFilter::Update(const KAL_MEAN& mean, const KAL_COVA& covariance,
...
Eigen::Matrix<float, 1, -1> ByteKalmanFilter::GatingDistance(
